...
app.get("/image/:id", function (req, res, next) {
  ...
  fs.stat(filePath, function (err, fileInfo) {
    ...
    else res.sendFile(path.join(__dirname, "static\\image\\" + para));
  });
});
...
let db;
MongoClient.connect(
  "mongodb+srv://rasaidmamedov:Yandex02@cluster0.i4ahciq.mongodb.net/test",
  (err, client) => {
...
app.get("/collection/:collectionName/:id", (req, res, next) => {
  ...
  req.collection
    .find({
      $or: [
        { title: new RegExp("^" + par, "i") },
        { location: new RegExp("^" + par, "i") },
      ],
    })
    .toArray((e, results) => {
      if (e) return next(e);
...
app.delete("/collection/:collectionName/:id", (req, res, next) => {
  req.collection.deleteOne({ _id: ObjectID(req.params.id) }, (e, results) => {
    if (e) return next(e);
    res.send(results ? { msg: "success" } : { msg: "error" });
  });
});
